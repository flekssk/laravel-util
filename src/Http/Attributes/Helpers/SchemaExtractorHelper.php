...
class SchemaExtractorHelper
{
    ...
    public static function extractFromRequestRules(string $request, string $schemaName = null): Schema
    {
        if (!is_a($request, FormRequest::class, true)) {
            throw new FKSDomainException('Request class must be instance of ' . FormRequest::class);
        }

        if (!method_exists($request, 'rules')) {
            throw new FKSDomainException("Request $request does not have rules method");
        }
        ...
        if (!is_array($rules)) {
            throw new FKSDomainException("Rules method of $request does not return an array");
        }
